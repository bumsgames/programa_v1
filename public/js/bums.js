    ...
    if ((nombre_cliente.length > 0 || apellido_cliente.length > 0 || cedula_cliente.length > 0 || nickname.length > 0)) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data.mensaje, function (i, item) {
                    nuevaFila += "<tr><td>" + item.name + " " + item.lastname + "</td><td>" + ((item.documento_identidad) ? item.documento_identidad : 'No disponible') + "</td><td>" + item.nickname + "</td><td><button id='bat' onclick='divFunction(\"" + item.id + "\",\"" + item.nickname + "\",\"" + item.name + "\",\"" + item.lastname + "\",\"" + item.num_contact + "\",\"" + item.note + "\",\"" + ((item.documento_identidad) ? item.documento_identidad : '') + "\",);'>Seleccionar</button></td></tr>";
                });
                $("#table_client").append(nuevaFila);
            }
        });
...
function divFunction(id, nickname, name, lastname, contact, note, documento_identidad) {
    ...
    $("#note").val(note);
    $("#cedula_cliente").val((documento_identidad!=null) ? documento_identidad : '');
    $("#table_client td").remove();
}
